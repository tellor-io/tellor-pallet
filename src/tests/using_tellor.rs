...
fn get_new_value_count_by_query_id() {
	...
	// Based on https://github.com/tellor-io/usingtellor/blob/cfc56240e0f753f452d2f376b5ab126fa95222ad/test/functionTests-UsingTellor.js#L47
	ext.execute_with(|| {
		with_block(|| {
			assert_ok!(Tellor::submit_value(
				RuntimeOrigin::signed(reporter),
		...
		with_block_after(REPORTING_LOCK, || {
			...
			assert_eq!(Tellor::get_new_value_count_by_query_id(query_id), 3);
		});
	});
}
...
fn get_timestamp_by_query_id_and_index() {
	...
	// Based on https://github.com/tellor-io/usingtellor/blob/cfc56240e0f753f452d2f376b5ab126fa95222ad/test/functionTests-UsingTellor.js#L56
	ext.execute_with(|| {
		let timestamp_1 = with_block(|| {
			assert_ok!(Tellor::submit_value(
				RuntimeOrigin::signed(reporter),
		...
		assert_eq!(Tellor::get_timestamp_by_query_id_and_index(query_id, 0), Some(timestamp_1));
		assert_eq!(Tellor::get_timestamp_by_query_id_and_index(query_id, 1), Some(timestamp_2));
		assert_eq!(Tellor::get_timestamp_by_query_id_and_index(query_id, 2), Some(timestamp_3));
	});
}
...
fn get_index_for_data_before() {
	...
	// Based on https://github.com/tellor-io/usingtellor/blob/cfc56240e0f753f452d2f376b5ab126fa95222ad/test/functionTests-UsingTellor.js#L68
	ext.execute_with(|| {
		with_block(|| {
			assert_ok!(Tellor::submit_value(
				RuntimeOrigin::signed(reporter),
		...
		assert_eq!(Tellor::get_index_for_data_before(query_id, timestamp_3), Some(1))
	});
}
...
fn get_data_before() {
	...
	// Based on https://github.com/tellor-io/usingtellor/blob/cfc56240e0f753f452d2f376b5ab126fa95222ad/test/functionTests-UsingTellor.js#L80
	ext.execute_with(|| {
		let timestamp_1 = with_block(|| {
			assert_ok!(Tellor::submit_value(
				RuntimeOrigin::signed(reporter),
		...
		assert_eq!(
			Tellor::get_data_before(query_id, timestamp_2).unwrap(),
			(uint_value(150), timestamp_1)
		)
	});
}
...
fn get_data_after() {
	...
	// Based on https://github.com/tellor-io/usingtellor/blob/cfc56240e0f753f452d2f376b5ab126fa95222ad/test/functionTests-UsingTellor.js#L162
	ext.execute_with(|| {
		let timestamp_0 = now();

		let timestamp_1 = with_block(|| {
			assert_eq!(Tellor::get_data_after(query_id, timestamp_0), None);
			assert_ok!(Tellor::submit_value(
				RuntimeOrigin::signed(reporter),
				query_id,
				uint_value(150),
		...
		assert_eq!(
			Tellor::get_data_after(query_id, timestamp_0),
			Some((uint_value(150).into_inner(), timestamp_1))
		);
		assert_eq!(Tellor::get_data_after(query_id, timestamp_1), None);
		...
		assert_eq!(
			Tellor::get_data_after(query_id, timestamp_1),
			Some((uint_value(160).into_inner(), timestamp_2))
		);
		assert_eq!(Tellor::get_data_after(query_id, timestamp_2), None);
	});
}
